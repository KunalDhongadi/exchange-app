        ...
        <div className="relative rounded-lg">
            ...
            <form className="space-y-6" method="POST" onSubmit={signupBtn}>
                ...
                <div>
                    ...
                    <input type="text" name="username" id="username" className="bg-gray-50 border border-zinc-300 text-gray-900 text-sm rounded-lg focus:ring-lime-200 focus:border-lime-200 block w-full p-2.5 dark:bg-zinc-800 dark:border-zinc-750 dark:placeholder-zinc-400 dark:text-white" placeholder="e.g.johndough" required onChange={onUsernameChange} value={username}/>
                </div>
                <div>
                    <label htmlFor="email" className="block mb-2 text-xs font-medium text-gray-900 dark:text-zinc-200">Email</label>
                    <input type="email" name="email" id="email" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-lime-200 focus:border-lime-200 block w-full p-2.5 dark:bg-zinc-800 dark:border-zinc-750 dark:placeholder-zinc-400 dark:text-white" placeholder="Enter your email address" required onChange={onEmailChange} value={email}/>
                </div>
                <div>
                    <label htmlFor="password" className="block mb-2 text-xs font-medium text-gray-900 dark:text-zinc-200">Password</label>
                    <input type="password" name="password" id="password" placeholder="Enter new password" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-lime-200 focus:border-blue-500 block w-full p-2 dark:bg-zinc-800 dark:border-zinc-750 dark:placeholder-zinc-400 dark:text-white" required onChange={onPasswordChange} value={password}/>
                </div>
                {!isLoading ? <button type="submit" className="w-full text-lime-900 bg-blue-700 focus:ring-4 focus:outline-none focus:ring-lime-100 font-medium rounded-lg text-sm px-5 py-2 text-center dark:bg-lime-200">Create account</button> : <LoadingSpinner/>}
